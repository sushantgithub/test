# laptop-speedup.ps1 — diagnose slowness and apply safe, reversible cleanups on Windows.
#
...
try {
    ...
    Write-Host ("Current load:   {0}%" -f $cpu.LoadPercentage)
...
try {
    ...
    Write-Host ("In use:    {0} ({1:N0}%)" -f (Format-Bytes ([long]$used), $pct))
...
Get-CimInstance Win32_LogicalDisk -Filter "DriveType=3" | ForEach-Object {
    ...
    Write-Host ("{0}  {1} free of {2} ({3}% used)" -f $_.DeviceID, (Format-Bytes $_.FreeSpace), (Format-Bytes $_.Size), $usedPct)
    if ($usedPct -ge 90) {
